...
import time
from tickers import tickers, futures, years, FINAM_URL
import futures
import os
import requests
...
def finam_parsing(mode, new_dir, ticker, start_day, end_day):
	...
	if mode == "1":
		...
		params = urlencode([
			('market', 0),  # на каком рынке торгуется бумага
			('em', futures[ticker]),  # вытягиваем цифровой символ, который соответствует бумаге.
			('code', ticker),  # тикер нашей акции
			('apply', 0),  # не нашёл что это значит.
			('df', start_day.day),  # Начальная дата, номер дня (1-31)
			('mf', start_day.month - 1),  # Начальная дата, номер месяца (0-11)
...
def down_min(stocks):
	if type(stocks) is str:
		...
		for year in years:
			...
			finam_parsing(mode, new_directory, stocks, start_date, end_date)
		...
		for item in stocks:
			...
			for year in years:
				...
				start_date = datetime(year, 1, 1).date()
				end_date = datetime(year, 12, 31).date()
				finam_parsing(mode, stock_dir, item, start_date, end_date)
...
if __name__ == '__main__':
	mode = input("Что грузить: акции - '1' или фьючерсы - '2'\n")
	if mode not in ('1', '2'):
